...
└── utils/
    ...
    ├── register-tool.ts          # registerTool() — FastMCP wrapper (input-mode JSON Schema, validation errors)
...
The server is built on [FastMCP for TypeScript](https://github.com/PrefectHQ/fastmcp-ts) (`@prefecthq/fastmcp-ts`), which wraps the official MCP SDK and owns transport + protocol wiring. Each tool file exports a `register*` function that takes a `FastMCP` instance and registers the tool through the shared `registerTool()` helper (`src/utils/register-tool.ts`), passing the tool's `z.object(...)` schema as `input`. The schema is also exported so tests can import the source of truth instead of redefining it.

```typescript
export const searchTendersInputSchema = z.object({/* fields */});
...
`registerTool()` exists because FastMCP's built-in handling of the Zod `input` differs from the raw SDK in two ways that hurt the client-facing contract, so we own both steps ourselves:

- **Advertised `inputSchema`** — FastMCP calls `z.toJSONSchema()` in _output_ mode, which lists fields with `.default()` (e.g. `lang`) under `required`. The helper generates the schema in _input_ mode (`{ io: "input" }`) so defaults stay optional.
- **Validation errors** — FastMCP rejects invalid arguments with a JSON-RPC `-32602` protocol error whose message drops the field paths. The helper runs `schema.safeParse()` itself and returns an `isError: true` tool result listing every issue with its path (`- projectId: Invalid UUID`), which the model can read and act on.
...
Tool handlers convert caught errors through `toToolErrorResult()` (`src/utils/errors.ts`), which returns a FastMCP `ToolResult` (`isError: true`) with a user-facing message that distinguishes:
