/**
 * Tests for browse_oag_tree tool
 */
...
describe("browse_oag_tree schema validation", () => {
  describe("parentCode parameter", () => {
    it("should accept valid OAG codes", () => {
      const result = schema.safeParse({ parentCode: "1" });
      expect(result.success).toBe(true);
    });

    it("should accept multi-digit codes", () => {
      const result = schema.safeParse({ parentCode: "11" });
      expect(result.success).toBe(true);
    });
    ...
    it("should reject codes with decimal points", () => {
      const result = schema.safeParse({ parentCode: "1.1" });
      expect(result.success).toBe(false);
    });
  });
...
describe("query parameter building", () => {
  ...
  it("should include parentCode when provided", () => {
    const result = buildQueryParams({ parentCode: "1" });
    expect(result.parentCode).toBe("1");
  });
