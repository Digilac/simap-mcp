 ...
 */
export const browseNpkTreeInputSchema = z.object({
  parentCode: z
    .string()
    .regex(/^[0-9]{1,10}$/)
    .optional()
 ...
 */
export function registerBrowseNpkTree(server: FastMCP): void {
  registerTool(
    server,
    {
      name: "browse_npk_tree",
      description: "Browse the NPK code hierarchy (standardized positions catalog)",
      input: browseNpkTreeInputSchema,
    },
    handler
  );
}
